...
public class TOCWS {
	...
	public void loadData(String text, Issue issue, ArrayList<Document> searchResultList) {
		...
		// TODO Auto-generated method stub
				...
				try {
					...
					loadResultList(documentRoot, issue, searchResultList);
	...
	private void loadResultList(JSONArray documentRoot, Issue issue, ArrayList<Document> searchResultList){
		...
		for (int i=0; i<documentRoot.length(); i++){
			...
			try {				
				...
				try {
					...
					r.setCol(SciELOAppsActivity.myConfig.getJcn().getItem(issue.getJournal().getCollectionId()));
				
					_pid = id;	
					_pid = _pid.replace("art-", "");
					_pid = _pid.replace("^c" + issue.getJournal().getCollectionId(), "");
