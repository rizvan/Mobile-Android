...
public class JournalSearchResult {
	...
	JournalSearchResult(String url, SciELONetwork jc, PairsList subjects, PairsList languages, ArrayList<Journal> searchResultList){
	...
	public void loadSearchResultList(){
		...
		for (int i=0; i<this.docs.length(); i++){
			...
			try {				
				...
				try {
					r.setTitle(  resultItem.getJSONArray("title").getString(0));	
				} catch (JSONException e) {
					last = last + "\n" +"ti";
				}
				try {
